...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not actions:
            return 0.0
        action = [self.getQValue(state, action) for action in actions]
        return max(action)
    ...
    def update(self, state, action, nextState, reward):
        ...
        if nextState:
            self.values[(state, action)] = (((1-self.alpha)*oldQ) + self.alpha*reward +
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in featureVector:
            sum += featureVector[feature] * weights[feature]
        return sum
